...
async function setupDownloadCapture(page) {
    const downloadPath = (0, fs_1.mkdtempSync)((0, path_1.join)((0, os_1.tmpdir)(), "n8n-puppeteer-"));
...
async function getDownloadedFiles(downloadPath, helpers) {
    const files = (0, fs_1.readdirSync)(downloadPath);
    const binaryData = {};
    for (const file of files) {
        const filePath = (0, path_1.join)(downloadPath, file);
...
async function runCustomScript(itemIndex, items, browser, page) {
    ...
    if (captureDownloads) {
        ...
        console.log(`Puppeteer node: Download capture enabled, files will be saved to: ${downloadPath}`);
    }
    const context = Object.assign(Object.assign({ $getNodeParameter: this.getNodeParameter, $getWorkflowStaticData: this.getWorkflowStaticData, helpers: Object.assign(Object.assign({}, this.helpers), { httpRequestWithAuthentication: this.helpers.httpRequestWithAuthentication.bind(this), requestWithAuthenticationPaginated: this.helpers.requestWithAuthenticationPaginated.bind(this) }) }, this.getWorkflowDataProxy(itemIndex)), { $browser: browser, $page: page, $puppeteer: puppeteer_extra_1.default, $input: {
            query,
        } });
    ...
    try {
        ...
        if (captureDownloads && downloadPath) {
            ...
            const downloadedFiles = await getDownloadedFiles(downloadPath, this.helpers);
            ...
            if (fileCount > 0) {
                ...
                normalizedItems[0] = Object.assign(Object.assign({}, normalizedItems[0]), { binary: Object.assign(Object.assign({}, (normalizedItems[0].binary || {})), downloadedFiles) });
            }
            cleanup();
        }
...
class Puppeteer {
    ...
    async execute() {
        ...
        const protocol = options.protocol ||
            process.env.PUPPETEER_PROTOCOL;
        ...
        const isContainer = (0, Puppeteer_node_options_1.isRunningInContainer)();
        let addContainerArgs = false;
        if (addContainerArgsOption === false) {
            console.log("Puppeteer node: Container arguments explicitly disabled by user");
            addContainerArgs = false;
        }
        else if (addContainerArgsOption === true ||
            addContainerArgsOption === undefined) {
            if (isContainer) {
                console.log("Puppeteer node: Container environment detected, applying container arguments");
                addContainerArgs = true;
            }
            else if (addContainerArgsOption === true) {
                console.log("Puppeteer node: Container arguments enabled (no container detected)");
                addContainerArgs = true;
            }
        }
        if (addContainerArgs) {
            ...
            if (missingContainerArgs.length > 0) {
                console.log("Puppeteer node: Adding container arguments:", missingContainerArgs);
                args.push(...missingContainerArgs);
            }
            else {
                console.log("Puppeteer node: Container arguments already present in launch arguments");
            }
        }
        ...
        try {
            if (browserWSEndpoint) {
                const isFromEnv = !options.browserWSEndpoint &&
                    (process.env.PUPPETEER_BROWSER_WS_ENDPOINT ||
                        process.env.PUPPETEER_WS_ENDPOINT);
                if (isFromEnv) {
                    console.log(`Puppeteer node: Using browser WebSocket endpoint from environment variable: ${browserWSEndpoint}`);
                }
